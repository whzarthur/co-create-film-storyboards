...
6. 写入成功后调用 `get_artifact`，核对服务端最终 `artifact_id`、revision、路径、hash、数据状态和警告，并确认本次镜号的文字、实际主图及适用的调度图引用与已确认版本一致。只有回读一致后才向作者简短报告“该镜已同步”，再进入下一镜。整集尚未完成时按契约表达阶段进度，不把单镜确认误标为全片完成。
